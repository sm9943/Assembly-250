...
;Calculation for F
            ...
            B       FStore           ;Branch to store the value of F
            
FZero       MOVS    R3,#0            ;Sets F = 0

FStore      STR     R3,[R0,#0]       ;Store the value of F in the variable
            
            
;Calculation for G
            LDR     R0,=G            ;Load the Address of variable G in R0
            ...
            LDR     R4,=const_G      ;Load the constant term for G in R4
            LDR     R4,[R4,#0]    
            ...
            B       GStore           ;Branch to store the value of G
            
GZero       MOVS    R3,#0            ;Sets G = 0

GStore      STR     R3,[R0,#0]       ;Store the value of G in the variable


;Calculation for Result
            LDR     R0,=F                ;Load the Address of variable F in R0
            LDR     R4,[R0,#0]           ;Load the value of F in R4
									     
            LDR     R0,=G                ;Load the Address of variable G in R0
            LDR     R5,[R0,#0]           ;Load the value of G in R5 
									     
            LSLS    R4,R4,#24            ;Shift the value of R4 to the MSB 
            LSLS    R5,R5,#24            ;Shift the value of R5 to the MSB 
									     
            ADDS    R3,R4,R5             ;F+G Addition on R4 and R5
            BVS     ResultZero           ;Check for overflow, branch to set zero
		                                 ;if overflow found
									     
            ASRS    R3,R3,#24            ;Right shift the value to its original position
									     
            B       ResultStore          ;Branch to store the result in the variable
            
ResultZero      MOVS    R3,#0            ;Sets Result = 0 

ResultStore    LDR     R0,=Result        ;Load the Address of variable Result in R0
               STR     R3,[R0,#0]        ;Store the value of Result in the variable
